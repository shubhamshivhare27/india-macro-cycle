...
def fetch_nifty_6m_change() -> dict:
    ...
    try:
        ...
        if isinstance(nifty.columns, pd.MultiIndex):
            nifty.columns = [col[0] if isinstance(col, tuple) else col
...
def fetch_credit_growth(manual_value=None) -> dict:
    ...
    if manual_value is not None:
        ...
        _ok(d, f"{val:.1f}%", None,
            ...
            "Non-food credit growth YoY — update from rbi.org.in/WSS monthly")
        logger.info(f"  ✅ Credit growth from manual input: {val}")
        return d
...
# ════════════════════════════════════════════════════════════
def fetch_bank_npa(manual_value=None) -> dict:
    d = _base("bank_npa_ratio", "Bank NPA Ratio", "%")
    if manual_value is not None:
        val = float(str(manual_value).strip())
        trend = "Healthy (Bullish)" if val < 3 else "Moderate" if val < 6 else "Stressed (Bearish)"
        _ok(d, f"{val:.1f}%", None,
            "Manual entry (weekly_inputs.json) — RBI FSR semi-annual",
            "https://rbi.org.in",
            "Latest semi-annual (RBI FSR)", trend,
            "Gross NPA ratio — RBI FSR released June & December")
        logger.info(f"  ✅ Bank NPA from manual input: {val}")
        return d
    npa_val = None
...
def fetch_all_indicators(gsheet_values: dict = None) -> dict:
    ...
    fetchers = [
        ...
        ("credit_growth",           lambda: fetch_credit_growth(gsheet_values.get("credit_growth"))),
        ...
        ("bank_npa_ratio",          lambda: fetch_bank_npa(gsheet_values.get("bank_npa_ratio"))),
